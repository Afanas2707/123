...
public class FieldPermissionService {
    ...
    private JsonNode getUISchemaForSession(UUID sessionId) {
        ChatMessage lastMessageWithTemplate = chatMessageRepository
                .findFirstBySessionIdAndTemplateIsNotNullOrderByCreationDateDesc(sessionId)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.BAD_REQUEST,
                        "Не найдена UI-схема для сессии " + sessionId + ". Невозможно проверить права."));

        Template template = lastMessageWithTemplate.getTemplate();
